...
#else
            if (UnityEngine.Input.GetKeyDown(KeyCode.Return)) RunCommand();
            if (UnityEngine.Input.GetKeyDown(KeyCode.UpArrow)) ReInsertCommand(false);
            if (UnityEngine.Input.GetKeyDown(KeyCode.DownArrow)) ReInsertCommand(true);
#endif
        }
