...
namespace qASIC.Console
{
    public static class GameConsoleController
    {
        ...
        static void Initialize()
        {
            ...
            Log($"qASIC Game Console System v{qInfo.ConsoleVersion}!", "console");
            Log("Successfully finished console controller initialization!", "init");

            qLogger.OnLogColor += (s, c) => Log(s, c);
            qLogger.OnLogColorTag += (s, c) => Log(s, c);
            qLogger.OnLogColorInternal += (s, c) => Log(GameConsoleLog.CreateNow(s, c, GameConsoleLog.LogType.Internal, true));
            qLogger.OnLogColorTagInternal += (s, c) => Log(GameConsoleLog.CreateNow(s, c, GameConsoleLog.LogType.Internal, true));
            ...
            if (_config.showThankYouMessage)
                Log($"Thank you for using qASIC v{qASIC.qInfo.Version}", "qasic");
        }
